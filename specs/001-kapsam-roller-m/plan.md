...
- **Storage**: Supabase Postgres with schema defined in `db/schema.sql`, migrations under `supabase/migrations/` (apply/sync via `supabase db push` or the equivalent Supabase CLI workflow), and seed data in `db/seed.mjs`.
...
supabase/
└── migrations/        # Supabase CLI migrations (source of truth for schema changes)
